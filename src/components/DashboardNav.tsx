...
import { BookOpen, XCircle, BarChart3, LogOut, User, Layers, TrendingUp, Target, Share2, Menu, X, Building2, Library, Briefcase } from 'lucide-react'
...
export default function DashboardNav({ profile, onOpenStats, blockType }: DashboardNavProps) {
  ...
  const isCorporateFinance = pathname?.startsWith('/corporate-finance')
  const navItems = isCorporateFinance
    ? [
        { href: '/choose-career', label: 'Careers', icon: Briefcase },
        { href: '/corporate-finance', label: 'Corporate Finance', icon: Target },
      ]
    : [
        { href: '/choose-career', label: 'Careers', icon: Briefcase },
    ...
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 backdrop-blur-md bg-[#0a0f1a]/90 border-b border-[#1f2937]">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-2 sm:py-4 flex items-center justify-between">
          <Link href="/choose-career" className="logo text-sm sm:text-xl">
            ESCP Finance +
          </Link>
          ...
          <div className="hidden md:flex items-center gap-1 sm:gap-2">
            ...
            <div className="relative ml-1 sm:ml-2" ref={menuRef}>
              <button
                ...
                className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-[#1f2937] border-2 border-[#374151] hover:border-[#2563eb] transition-colors overflow-hidden flex items-center justify-center"
              >
                {profile?.avatar_url ? (
                  <img
          ...
          <div className="md:hidden flex items-center gap-2">
            <button
              ...
              className="w-8 h-8 rounded-lg bg-[#1f2937] border border-[#374151] hover:border-[#2563eb] transition-colors flex items-center justify-center text-white"
            >
              {showMobileMenu ? (
                <X className="w-5 h-5" />
              ) : (
