...
export default function QuantPage() {
  ...
  const categories = Object.keys(quantBlockQuestions) as Array<keyof typeof quantBlockQuestions>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {categories.map((categoryId) => {
              ...
              const questions = quantBlockQuestions[categoryId]
              const questionCount = Array.isArray(questions) ? questions.length : 0
              
              return (
                <Link
                  key={categoryId}
                  href={`/quant/${categoryId}`}
                ...
                >
                  ...
                  >
                    <Icon className="w-5 h-5 sm:w-6 sm:h-6 text-[#10b981]" />
                  </div>

                  <h2 className="text-lg sm:text-xl font-bold mb-2">{quantCategoryLabels[categoryId]}</h2>
                  
                  <div className="text-xs sm:text-sm text-[#6b7280] mb-4">
                    {questionCount} question{questionCount !== 1 ? 's' : ''}
                  </div>

                  <div className="flex items-center gap-2 text-sm font-medium group-hover:gap-3 transition-all text-[#10b981]">
