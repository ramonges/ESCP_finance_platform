...
import { tradingBlockQuestions, tradingCategoryLabels } from '@/data/tradingBlockQuestions'
...
// Map trading block categories to original category types for stats
const categoryMapping: Record<string, Category> = {
  'behavioral': 'behavioral',
  'mental-calculation': 'mental-maths',
  'proba-exercises': 'proba',
  'brainteaser': 'proba', // Brainteasers are from proba
  'trading-intuition': 'trading',
  'ml-questions': 'ml-ai',
}

export default function TradingCategoryTrainingPage() {
  ...
  const questions: Question[] = tradingBlockQuestions[categoryId as keyof typeof tradingBlockQuestions] as Question[] || []
  const categoryLabel = tradingCategoryLabels[categoryId] || categoryId
  ...
  }, [userId, currentQuestionIndex, questions, categoryId, originalCategory, supabase, reloadStats])
    ...
    return (
      <div className="min-h-screen gradient-bg flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/trading" className="text-[#2563eb] hover:underline">
            Back to Trading Categories
          </Link>
        </div>
      </div>
    )
      ...
      <main className="pt-16 sm:pt-24 pb-8 sm:pb-12 px-3 sm:px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-4 sm:mb-6">
            ...
            <span className="text-[#9ca3af] text-xs sm:text-sm mx-2">/</span>
            <span className="text-[#2563eb] text-xs sm:text-sm font-medium">{categoryLabel}</span>
          </div>
          ...
          {isComplete ? (
            ...
            <QuestionCard
              key={`trading-${categoryId}-${currentQuestion.id}`}
