...
import { Loader2, ArrowRight, Brain, Calculator, TrendingUp, Users, Cpu, Puzzle } from 'lucide-react'
...
const categoryIcons: Record<string, React.ElementType> = {
  'behavioral': Users,
  'mental-calculation': Calculator,
  'proba-exercises': Brain,
  'brainteaser': Puzzle,
...
export default function TradingPage() {
  ...
  const categories = Object.keys(tradingBlockQuestions) as Array<keyof typeof tradingBlockQuestions>
      ...
      <main className="pt-16 sm:pt-24 pb-12 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <div className="mb-8 sm:mb-12">
            <Link 
              href="/select-block"
              className="text-[#9ca3af] hover:text-[#2563eb] text-sm mb-4 inline-block transition-colors"
            >
              ← Back to Blocks
            </Link>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {categories.map((categoryId) => {
              ...
              const questions = tradingBlockQuestions[categoryId]
              const questionCount = Array.isArray(questions) ? questions.length : 0
              
              return (
                <Link
                  key={categoryId}
                  href={`/trading/${categoryId}`}
                ...
                >
                  ...
                  <h2 className="text-lg sm:text-xl font-bold mb-2">{tradingCategoryLabels[categoryId]}</h2>
                  
                  <div className="text-xs sm:text-sm text-[#6b7280] mb-4">
                    {questionCount} question{questionCount !== 1 ? 's' : ''}
                  </div>

                  <div className="flex items-center gap-2 text-sm font-medium group-hover:gap-3 transition-all text-[#2563eb]">
